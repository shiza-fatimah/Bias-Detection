...
#input_dir = '../misc/'
input_dir = os.path.normpath(os.path.join(current_directory, '..', 'misc'))
output_dir = os.path.normpath(os.path.join(current_directory, '..', 'results'))
logs_dir = os.path.join(output_dir, 'logs/')
output_pickle_dir = os.path.join(output_dir, 'pickle')

output_df_path =os.path.join(output_dir, 'allsides-df.csv')
...
# Get all user agents
user_agents = []
agent_file_path = os.path.join(input_dir, 'user-agents.txt')
...
def convert_to_df(data):
    ...
    for story_grouped in stories:
        grouped_with = list(range(all_stories_counter, all_stories_counter + len(story_grouped)))

        for story in story_grouped:
            story_misc = story[6]  # author, time, topics

            story_time = story_misc.get("time", "")
            story_bias = story[0].replace("From the", "").strip()
            story_content = story[5]
            story_title = story[4]
            story_topic = story_misc.get("topics", "")
...
# logging
all_errors_new = {}
file_errors_new = open(os.path.join(logs_dir, 'errors-new.txt'), 'w+', encoding='utf8')
all_errors_parsing = {}
...
stories_csv_file = os.path.join(output_dir, "stories.csv")
...
with open(stories_csv_file, encoding='utf8', newline='') as f:
    ...
    for story_data in reader:
        ...
        featured_coverage = story_soup.find('div', class_='featured-coverage')
        if featured_coverage:
            news_bias_list = featured_coverage.find_all('h3')
            news_title_links = featured_coverage.find_all('a', class_='news-title')
            news_body_list = featured_coverage.find_all('div', class_='body-contents')
            news_sources = featured_coverage.find_all('div', class_='news-source')
        else:
            print("Error: 'featured-coverage' section not found in the HTML.")
        ...
        for i in range(news_sources_size):
            ...
            try:
                news_bias = news_bias_list[i].text.strip()
            except Exception as e:
                print(f"Error extracting news_bias: {e}")
            
            try:
                news_title = news_title_links[i].text.strip()
                news_link = news_title_links[i]['href'].strip()
            except Exception as e:
                print(f"Error extracting news_title or news_link: {e}")
            
            try:
                paragraphs = news_body_list[i].select('p')
                if paragraphs:
                    news_body = ' '.join([p.text.strip() for p in paragraphs])
            except Exception as e:
                print(f"Error extracting news_body: {e}")
            
            try:
                news_source = news_sources[i].find('span').text.strip()
            except Exception as e:
                print(f"Error extracting news_source: {e}")
                ...
                # Success, save article
                    ...
                    if oriBody == '':
                        ...
                        if 'time' in oriMisc:
                            ...
                            if len(fix_1) > 0:
                                time = re.sub(r"([\(\[]).*?([\)\]])", "", time).strip().replace('  ',' ')
                                time = time[:2] + ':' + time[2:]
...
newpkl_file_path = os.path.join(output_pickle_dir, new_pickle_file)
with open(newpkl_file_path, 'wb') as f:  
